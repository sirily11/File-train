...
contract FileStorage {
    ...
    struct FilePermissions {
        string fileName;
        string[] PermissionsAdress;
    }

    // store the file metadata with user's address
    mapping(address => FileMetaData[]) _files;
    // FID: whitelist
    mapping(string => FilePermissions) _permissions;



    // store the file data with fileId
    // function addFile(FileMetaData memory file, string[] memory EFid, string[] memory group) public {
    function addFile(FileMetaData memory file, string[] memory group) public {
        _files[msg.sender].push(file);
        _permissions[file.fileId].PermissionsAdress = group;
        _permissions[file.fileId].fileName = file.fileName;
    ...
    function check_permission(string memory fileId, string memory senderId) public view returns (string memory) {
         for (uint i = 0; i < _permissions[fileId].PermissionsAdress.length; i++) {
            if (keccak256(abi.encodePacked(senderId)) == keccak256(abi.encodePacked(_permissions[fileId].PermissionsAdress[i]))) {
                return _permissions[fileId].fileName;
            }
        }
        return "";
    }
